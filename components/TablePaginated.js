...
export default function TablePaginated({ header=[], body=[] }) {
    ...
    const mapping = (key) => _map[key] || key
    ...
    const onPageClick = (e) => {
        ...
        {   
            ...
            if(pagination>((lines/visible)-3))
                pagination=((lines/visible)-3)
    ...
    return (
        <>
            ...
            >
                ...
                <tbody>
                    {body.map( (b=[],b_idx) => <tr key={b_idx} style={{display:b_idx<(state.page*state.visible)&&b_idx>=((state.page-1)*state.visible)?'table-row':'none'}}>{b.map( (d,d_idx) => <td key={b_idx*state.cols+d_idx} data-index={b_idx*state.cols+d_idx}>{mapping(d)}</td>)}</tr> )}
                </tbody>
            </table>
            <nav className="pagination is-centered is-small" role="navigation" aria-label="pagination">
                <ul className="pagination-list">
                    ...
                    {body.slice(0,(body.length/state.visible)+1).map( (_,idx) => <li key={`l-${idx}`}><a key={`p-${idx}`} data-index={idx+1} onClick={onPageClick} className={["pagination-link",idx==(state.page-1)?'is-current':''].join(' ')}>{idx+1}</a></li>).slice((state.pagination-3),state.pagination)}
                    ...
                    {body.slice(0,(body.length/state.visible)+1).map( (_,idx) => <li key={`l-${idx}`}><a key={`p-${idx}`} data-index={idx+1} onClick={onPageClick} className={["pagination-link",idx==(state.page-1)?'is-current':''].join(' ')}>{idx+1}</a></li>).slice(-4,-1)}
                </ul>
            </nav>
        </>
    );
